...
**IA note (restructure):** docs renumbered **01–19 to reading order**; the menu is **10 chapters** (Start here + 9, incl. a '5 · Care pathways & delivery' chapter and a 'Gaps, blind-spots & roadmap' chapter); each appendix catalog now embeds its sortable **"Spreadsheet view"** so there are **no standalone `grid-*` pages**; the static **DFD was folded into the Calculation Explorer**; **male+female → `purescore-sex.html`** (segmented toggle) and **class diagram + class explorer → `class-model.html`** (`build_class_model` wraps `build_behemoth` + `build_class_explorer`).

**Care pathways & delivery chapter (NEW):** condition care-pathways from the Abu Dhabi / PURA care plans, integrated data-driven. Source: `data/care-pathways.json` (8 pathways: IEAT lifecycle, 3-tier priority→role, referral, monitoring, UAE specifics, **threshold reconciliation vs canonical** — aligned/partial/divergent), `data/care-roles.json` (hybrid human-in-the-loop roles), `data/prevention-nudges.json` (PURA reminders, interventions, alerts, CDS, Prevention Score, KPIs). Rendered server-side by `build_care_pathways` / `build_care_roles` / `build_prevention`. `build_wiki.py:_care_guard()` HARD-FAILS the build if a pathway references an unknown role, an unknown priority owner, or an invalid reconciliation status. **Reconcile new pathway thresholds against the canonical ranges (calc-graph/pillars/range-variations) and flag divergences — never let a pathway cut-point silently contradict the engine.**
...
- `purescore-system.html` (**System at a glance** — board-facing whole-system uber map: ONE 6-module palette (intake·engine·action·safety·governance·outcome) shown 3 ways via view-tabs — Flow · Trust & Safety · Modules — plus peelable "Show" layer chips; `.body.html` fragment) ← the whole platform; it is the board-altitude entry that drills DOWN into the detailed System & build pages. Update its module dossier / lenses when a module's responsibility or a cross-module flow (intake→engine→action→outcome, safety firewall, actuarial firewall, governance gates) changes.
- **Multi-view mermaid gotcha (STANDING RULE for `purescore-system` / `engagement-state-machines` and any new tabbed diagram):** mermaid runs `startOnLoad` with `htmlLabels:true` (see `MERMAID_HEAD`) and is a **module import — NOT on `window`**, so you cannot call `mermaid.run()` from a page script. HTML labels must be measured **while the diagram is visible**; hiding an inactive tab on a **fixed timer** races mermaid and leaves the non-default tabs as **error (viewBox ~`0 0 2412 512`) or collapsed (`-8 -8 16 16`) SVGs**. Correct pattern: keep ALL `.nv-view`s visible on load and only hide the inactive ones **after polling that every view has a valid rendered SVG** (`rendered()`/`allRendered()` in those `.body.html` files). Verify tabbed diagrams in a real browser (hidden-tab render), not just with mermaid-cli per-block (which always renders visible and hides this bug).
- `engagement-state-machines.html` ← Doc 11 (nudge), Appendix adherence (`data/adherence.json`), Docs 13/16/05. Its **'1 · Nudge engine'** section is a multi-view diagram (lifecycle · firewall+channels · when · gates, with focus filters) reflecting Doc 11 §9 delivery — update when §9 changes.
