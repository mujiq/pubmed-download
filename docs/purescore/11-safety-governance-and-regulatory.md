...
> suicidality rule (Doc 02 MCS), the cohort/fairness/calibration machinery (Doc 09), the gated
> clinician layer (Doc 08), the actuarial/pricing layer (Doc 10), the sex-specific reproductive
> models (Doc 05), and the genetic/aging inputs (Doc 04, Doc 10).
>
...
(README §1, §5.1; Doc 00 §3.1 anti-Babylon). Every critical/red state escalates to a human
...
(Doc 08 §1.1). Numerical clinical risk estimates (ASCVD, KDIGO, FIB-4, FRAX, KFRE, PhenoAge …)
live **only** in the gated clinician layer (Doc 08) and are never surfaced to the patient as a
diagnosis (§1.5).
...
software's output). The Doc 08 clinician layer is engineered to satisfy (3) and especially (4): every
clinical score ships with inputs, source guideline, model version, bands, and OOD flags so the
clinician can independently reproduce and overrule it (Doc 08 §3–§4). **A score the clinician cannot
...
| Drives a clinician decision the clinician can't independently review | Loses CDS exemption → device | Re-architect for inspectability (Doc 08 §4) or pursue device pathway |
...
The Doc 08 clinical-scores layer is a **separate product surface** with separate controls:
(1) access restricted to **credentialed clinicians** (and, where lawful and separately governed,
payers per Doc 10) via role-based access (§4.3); (2) **never** rendered to patients as diagnosis;
(3) every output **inspectable** to preserve the CDS posture (§1.2b); (4) its own validation,
OOD-flagging, and audit (Doc 08 §6, Doc 09, §5 here). Patient and clinician layers have **distinct
...
| **routine** (reserve-deficit red) | Very low VO2max, osteoporosis (FRAX), sarcopenia, chronic sub-optimal | Priority-improvement plan (Doc 06) + high-leverage nudges (Doc 07) | Flag for next visit; care-gap registry (Doc 00 §3.2) | **Next scheduled visit** |
...
Doc 08 §2.6) — forces MCS to **red/critical irrespective of the total**, is **never averaged away**
...
syncope, severe dyspnea, suicidal ideation, signs of sepsis, reproductive red-flags per Doc 05 such
...
Doc 00 §3.1): it routes red-flags to humans rather than adjudicating them.
...
- **Missing / low-coverage data** never produces a green reassurance. Low coverage caps how *green*
  a pillar may display (Doc 03 §3; Doc 01 §4.3); a pillar green only via imputed medians is shown as
...
- **Validated clinical scores feed the `max`**, never relax `r_i`, never clear a critical pillar,
  never lift `PURE_CRIT_CAP` (Doc 08 §3 invariants).
...
- **No silent imputation into clinical scores** — an under-determined clinical score is *not
  computable*, never guessed (Doc 08 §3.1).

The forbidden state — *telling a patient they are fine in a way that could mask an emergency* — is
designed out (Doc 00 §3.1). Where the model is unsure, it **says so and escalates / recommends
...
- **The system never overrides a human.** PureScore is one input to a care team, never the
  decision-maker (Doc 00 §3.3 Mayo team-based model).
...
| F10 | **Over-claiming / scope creep into SaMD** | Patient acts on an unvalidated "diagnosis" | 5 | 2 | 3 | Claims review gate (§1.4, §7.3); intended-use control; clinician-layer gating (§1.5); validation-before-scale (Doc 09) |
| F11 | **Acute-event mishandling** | Wrong re-prioritization during a real event | 4 | 2 | 3 | Acute-mode hysteresis + revert (Doc 06); clinician confirmation for care actions; escalation unaffected by acute weights |
| F12 | **Nudge harm / dark pattern** | Unsafe or manipulative recommendation | 4 | 2 | 3 | Contraindication screening on actions (Doc 07 §1); anti-overpromise (Doc 00 §3.1); evidence-weighted, attributed Δ (Doc 07) |
...
- **Collect only what a pillar/marker or a consented feature needs** (Core/Peripheral/Comprehensive
  tiering, Doc 01, already limits collection to high-yield measurements).
- **Purpose limitation:** scoring data is not repurposed (e.g. into Doc 10 pricing) without a
  **separate, explicit, revocable consent**; payer/actuarial use is separately gated (§1.5, Doc 10).
...
- **Role-based / attribute-based access control:** patient (own data), clinician (assigned patients,
  Doc 08 gated layer), payer (separately governed, de-identified/aggregated where possible, Doc 10),
...
### 4.4 Genetic data — special handling (GINA; tie to Doc 10)

Genetic inputs (epigenetic clocks/DNAm, any germline data feeding Doc 04 aging reservoirs or Doc 08
BioAge) get the **strictest** controls:

- **GINA firewall:** genetic information is **structurally walled off from the actuarial/pricing
  layer (Doc 10)** and from any employment-related use. It may inform *wellness and clinical
  interpretation* with consent, but **must not** flow into underwriting, pricing, or access decisions
  (README §5.5; Doc 10 fairness flags). This is enforced by access control, not policy alone.
...
### 4.5 Reproductive data — special handling (tie to Doc 05)

Sex-specific reproductive data (cycle, fertility, **pregnancy**, post-partum, menopause; Doc 05) is
...
- Reproductive **red-flags** (Doc 05) still escalate to humans (§2.3) — safety is never traded for
...
### 5.1 Versioning and change control (binding on Doc 03 §8, Doc 09)
...
> dataset is a version bump that requires re-validation (Doc 09) before production and is recorded
> in the audit trail.**
...
| **Bands / reference ranges** | Doc 02 green/yellow/red, Doc 01 cohort ranges | Bump + clinical sign-off + calibration check (Doc 09) |
| **Clinical-score equations** | ASCVD/KDIGO/FIB-4/FRAX versions (Doc 08) | Bump + OOD-envelope re-check (Doc 08 §6) |
| **Cohort/training datasets** | NHANES/Biobank refresh, percentile tables (Doc 09) | Bump + re-fit + fairness + drift re-audit |
| **Action library** | Doc 07 actions, effects, contraindications | Bump + safety screen |

Releases follow **change control**: proposal → review (clinical + technical + fairness) → validation
gates (Doc 09) → sign-off by named owners (§5.6) → staged rollout → monitoring. No "silent" constant
edits.

### 5.2 Production monitoring

Continuously monitor (ties to Doc 09 drift/calibration):
calibration vs realized outcomes; **input drift** and population shift; **OOD rate** (Doc 08 §6);
...
performance for **fairness** (§6, Doc 09); error and missing-data rates. Thresholds trip alerts to
...
subgroup** (Doc 09); known limitations and failure modes (§3); confidence/coverage semantics
(Doc 03 §3); version, date, and owners (§5.6). This is the anti-Babylon transparency artifact
(§7, Doc 00 §3.1): claims are bounded by validated evidence, in writing.
...
and fairness results before launch (Doc 09, §8); serious incidents and breaches (§4.6, §5.3); the
genetic/reproductive/actuarial firewall (§4.4–§4.5, Doc 10); and the appeal/contest process (§6.4).
The board can **block a launch** (§8) and is independent of commercial targets (Doc 00 §4 rejects
...
README §5.5 and Doc 00 §2.8 are binding: **no protected-class attribute (race, ethnicity, sex where
...
- **Scoring (Doc 09):** cohorting must **reduce, not encode** disparity; fairness audits (calibration
  and error parity across sex, age, race/ethnicity, SES) are **gates**, not afterthoughts; proxy
  detection is run on features.
- **Pricing (Doc 10):** the actuarial layer carries **heavy regulatory/fairness flags**; genetic info
  is firewalled from underwriting (§4.4, GINA); profit comes from **health improvement**, never from
  denial or proxy discrimination (Doc 00 §4).

Where guideline equations legitimately use sex/age/ancestry as *clinical* inputs (e.g. ASCVD,
sex-specific bands Doc 05), they are used **only** to improve clinical accuracy within the
clinician layer, never to worsen access/price, and OOD/fairness-checked (Doc 08 §6, Doc 09).
...
(Doc 00 §3.3 Mayo communication standard). Numbers come with meaning ("at-risk band — worth a
...
**If we cannot explain a number, we do not show it** (Doc 00 §2.2). Clinicians additionally get the
inspectable CDS surface (Doc 08 §4) that preserves the device exemption (§1.2b).

### 7.2 Published methodology and external audit

This specification (Docs 00–11) is the **published methodology**. Bands, formulas, constants,
...
The anti-Babylon rule (Doc 00 §3.1): **no claim ships ahead of its evidence.** Every patient- or
...
"impossible to lose money", Doc 00 §4). Intended use is set by claims; claims review is therefore a
...
|---|------|----------------|-------|-----|
| G1 | **Validation** | Prospective calibration meets target for the deployed population; no un-validated claim | Clinical + Technical | Doc 09; Doc 00 §3.1 |
...
| G7 | **Fairness** | No protected-class proxy; calibration/error parity across subgroups within tolerance | Fairness + Technical | §6; Doc 09; Doc 10 |
...
| G11 | **Explainability** | Every shipped score emits the Doc 03 §7 object; clinician CDS inspectable | Technical + Clinical | §7.1; Doc 08 §4 |
...
*Cross-references: README §5 (hard non-negotiables); Doc 00 §3.1 (anti-Babylon), §4 (no
...
Doc 04 (reservoirs, aging/genetic burdens); Doc 05 (reproductive/sex-specific data and red-flags);
Doc 06 (acute-mode); Doc 07 (nudge safety); Doc 08 (gated clinician CDS layer, OOD, inspectability);
Doc 09 (cohorts, calibration, fairness, drift, validation gates); Doc 10 (actuarial/pricing
